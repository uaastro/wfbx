...
static void print_help(const char* prog)
{
  printf(
    ...
    "\nExamples:\n"
    "  sudo %s wlan0\n"
    "  sudo %s --ip 239.0.0.1 --port 5600 wlan0 wlan1\n",
...
static int parse_cli(int argc, char** argv, struct cli_cfg* cfg)
{
  ...
  txf_parse(&cfg->txf, "any");   /* default: accept all */
  cfg->tx_filter_spec = "any";
  cfg->link_id = -1;
  cfg->group_id = -1;
  cfg->radio_port = -1;
  cfg->n_if = 0;
...
int main(int argc, char** argv)
{
  ...
  fprintf(stderr, " -> UDP %s:%d | stats %d ms | filters: accept-all | stats -> %s:%d id=%s\n",
...
stats_tick:
    ...
    if (t1 - t0 >= (uint64_t)cli.stat_period_ms) {
      ...
      /* Filters disabled: advertise accept-all. */
      ...
      if (!build_failed) {
        wfbx_rx_filter_info_t filters = {
          .group_id = 0xFF,                 /* unused */
          .txf_mode = TXF_ANY,              /* explicit accept-all */
          .has_link_id = 0,
          .has_radio_port = 0,
          .link_id = 0,
          .radio_port = 0,
