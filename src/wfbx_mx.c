  ...
  for (int t=0;t<MAX_TX_IDS;++t) {
    ...
    for (int i=0;i<n_if;i++) {
      ...
      for (int c=0;c<RX_ANT_MAX;c++) {
        ...
        AS->rssi_sum = 0;
        AS->rssi_samples = 0;
        memset(AS->ports, 0, sizeof(AS->ports));
      }
    }
  }
  for (int i=0;i<n_if;++i) {
        ...
        for (int tX = 0; tX < MAX_TX_IDS; ++tX) {
          ...
          for (int i2 = 0; i2 < n_open; ++i2) {
            ...
            for (int c = 0; c < RX_ANT_MAX; ++c) {
              ...
              uint32_t packets_clamped = (packets_ch > 0xFFFFu) ? 0xFFFFu : (uint32_t)packets_ch;
              uint32_t lost_clamped = (lost_ch > 0xFFFFu) ? 0xFFFFu : (uint32_t)lost_ch;
              chain->packets = clamp_u16(packets_clamped);
              chain->lost = clamp_u16(lost_clamped);
        ...
        for (int tX=0;tX<MAX_TX_IDS;tX++) {
          ...
          for (int i2=0;i2<n_open;i2++) {
            ...
            for (int c=0;c<RX_ANT_MAX;c++) {
              ...
              fprintf(stderr, "      ANTa%02d: rssi min=%d avg=%.1f max=%d | lost=%llu pkts=%llu\n",
                      (int)AS->ant_id,(int)AS->rssi_min,av,(int)AS->rssi_max,
                      (unsigned long long)lost_ch,(unsigned long long)packets_ch);
            }
            if (cli.debug_stats) {
